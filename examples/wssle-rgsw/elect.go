...
// DecodeResult reads the elected commitment h* off [Elect]'s decrypted output:
// it rounds the fragments at X^0 .. X^(C-1) and joins them.
//
...
//
// The rounding is done in integers ([RoundCoeffs]): a fragment is wider than
...
func DecodeResult(params CircuitParams, pt *rlwe.Plaintext) *big.Int {
	...
	for k := range frags {
		frags[k] = rounded[k].Abs(rounded[k])
	}
