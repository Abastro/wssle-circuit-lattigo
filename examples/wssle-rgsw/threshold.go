...
// fail decryption.
//
// A Gaussian would let the m floods add in variance, sqrt(m)*sigma rather than
// m*F, which is about half a bit cheaper here -- but only in the semi-honest
// analysis. The decryption proof certifies a norm bound on a member's flood and
// nothing about its distribution, so a malicious member may sit at the bound
// with the worst signs, and the budget is m times the bound whatever the
// flooding is. A Gaussian hiding to the same 2^-s needs sigma = F/2, so its
// support is 3F: it would widen the bound the proof certifies, and the variance
// argument buys nothing against it.
